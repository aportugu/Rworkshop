...
## We can also pipe the output of the survit object into the tbl_survfit() function using the %>% operator
survfit(Surv(Days.to.DLC, Death) ~ CAR.T.Product.Type , data = db) %>%
  tbl_survfit(
    times = 365,
...
### 3d. Do the same thing for progression-free survival.
```{r}
...
### 3e. Look at OS and PFS stratified by best response.
```{r}
## Stratify OS by the "Best.response" variable
ggsurvplot(survfit(Surv(Days.to.DLC, Death) ~ Best.response, data = db),
           pval=TRUE,
           conf.int = TRUE,
           risk.table = TRUE, # Add risk table
...
## Stratify PFS by the "Best.response" variable. Filter the data to exclude "PD."
ggsurvplot(survfit(Surv(Days.to.relapse.death.or.DLC, Relapse.or.death) ~ Best.response, data = filter(db, Best.response != "PD")),
           pval=TRUE,
           conf.int = TRUE,
           risk.table = TRUE, # Add risk table
           risk.table.col = "strata", # Change risk table color by groups
