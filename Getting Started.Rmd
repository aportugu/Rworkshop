...
temp.table.data <- db %>%
  transmute( ## Use the transmute() function to make readable variable names for the table.
    ...
    "Product" = factor(recode(CAR.T.Product.Type, "Breyanzi"= "Liso-cel", "Yescarta" = "Axi-cel"), levels = c("Liso-cel","Axi-cel")) ## Use the recode() function to change "Breyanzi" to the generic, "Liso-cel" and then use the factor() function to set the order of variables
  ) 
...
temp.table.data %>%
  tbl_summary(
    missing = "no", ## Ensures that missing data is not incliuded in the table
  ...
  add_stat_label() ## Used to add the statistical summaries ot the table (i.e., median with IQR)
```
